...
int insercaoPorCor(Pokemon *array, int n, int cor, int h) {
    for (int i = (h + cor); i < n; i+=h) {
        Pokemon tmp = array[i];
        int j = i - h;
        while ((j >= 0) && (getWeight(&array[j]) > getWeight(&tmp))) {
            comp++;
            array[j + h] = array[j];
            j-=h;
        }
        comp+=2;
        if(getWeight(&array[j]) == getWeight(&tmp) && strcmp(getName(&array[j]), getName(&tmp)) > 0) {
            array[j + h] = array[j];
            j-=h;
        }
        array[j + h] = tmp;
