...
public class SelectionSortPartial {
    ...
    private static void selectionSortIterative(List<Pokemon> list) {
        int n = list.size();
        for (int startIndex = 0; startIndex < n - 1; startIndex++) {
            // Encontra o índice do menor elemento na parte não ordenada
            int minIndex = startIndex;
            for (int i = startIndex + 1; i < n; i++) {
                if (list.get(i).getName().compareToIgnoreCase(list.get(minIndex).getName()) < 0) {
                    minIndex = i;
                }
            }

            // Troca o elemento mínimo encontrado com o primeiro elemento da parte não
            // ordenada
    ...
    public static void main(String[] args) {
      ...
      selectionSortIterative(selectedPokemons);
