...
class Show {
    ...
    public Show clone() {
        Show clone = new Show(this.show_id, this.type, this.title, this.director, this.cast, this.country,
...
class NoAno {
    ...
    NoAno esq, dir;
    NoTitulo raizTitulos;

    public NoAno(int chave) {
        this.chave = chave;
        this.esq = this.dir = null;
        this.raizTitulos = null;
    }
}
...
class ArvoreDeArvores {
    ...
    public void inserir(Show s) {
        ...
        raiz = inserirAno(raiz, chave, s);
    }

    private NoAno inserirAno(NoAno i, int chave, Show s) {
        if (i == null) {
            i = new NoAno(chave);
            i.raizTitulos = inserirTitulo(i.raizTitulos, s);
        } else if (chave < i.chave) {
            i.esq = inserirAno(i.esq, chave, s);
        } else if (chave > i.chave) {
            i.dir = inserirAno(i.dir, chave, s);
        } else {
            i.raizTitulos = inserirTitulo(i.raizTitulos, s);
        }
        return i;
    ...
    private NoTitulo inserirTitulo(NoTitulo i, Show s) {
        if (i == null) {
            return new NoTitulo(s);
        } else if (s.getTitle().compareTo(i.elemento.getTitle()) < 0) {
            i.esq = inserirTitulo(i.esq, s);
        } else if (s.getTitle().compareTo(i.elemento.getTitle()) > 0) {
            i.dir = inserirTitulo(i.dir, s);
        }
        return i;
    }
    ...
    public void pesquisar(String title) {
        ...
        System.out.println(pesquisarTitulo(raiz, title) ? "SIM" : "NAO");
    }

private boolean pesquisarTitulo(NoAno i, String title) {
    if (i == null) return false;

    if (pesquisarNaSub(i.raizTitulos, title)) {
        return true;
    }

    System.out.print("esq ");
    if (pesquisarTitulo(i.esq, title)) return true;

    System.out.print("dir ");
    return pesquisarTitulo(i.dir, title);
}


    private boolean pesquisarNaSub(NoTitulo i, String title) {
        if (i == null)
            return false;
        ...
        if (cmp == 0) {
            ...
            System.out.print("ESQ ");
            return pesquisarNaSub(i.esq, title);
        } else {
            System.out.print("DIR ");
            return pesquisarNaSub(i.dir, title);
        }
    }
...
public class Main {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        ArvoreDeArvores arvore = new ArvoreDeArvores(); // nova estrutura
        ...
        while (!getId.equals("FIM")) {
            ...
            for (int i = 0; i < shows.length; i++) {
                if (shows[i].getShow_id().equals(getId)) {
                    encontrado = shows[i];
                    break;
                }
            }
