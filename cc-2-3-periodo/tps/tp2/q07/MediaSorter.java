...
class MediaContent {
    ...
    public MediaContent(String id, String contentType, String title, String director, String[] castMembers, 
                       String country, LocalDate addedDate, Integer releaseYear, String rating, 
    ...
    public void display() {
        ...
        System.out.println("=> " + id + " ## " + title + " ## " + contentType + " ## " + director 
            ...
            + " ## " + rating + " ## " + duration + " ## [" + genresToString() +"] ##");
    }
    ...
    public MediaContent copy(){
        return new MediaContent(this.id, this.contentType, this.title, this.director, 
                              ...
                              this.releaseYear, this.rating, this.duration, this.genres);
    }
}

public class MediaSorter {
    public static void insertionSort(MediaContent[] array, Integer size){
        File logFile = new File("./853431_insercao.txt");
        try{
            ...
            for(int i = 1; i < size; i++){
                ...
                int j = i - 1;
                while(j >= 0 && 
                        (temp.getContentType().compareToIgnoreCase(array[j].getContentType()) < 0 ||
                        (temp.getContentType().compareToIgnoreCase(array[j].getContentType()) == 0 &&
                         temp.getTitle().compareToIgnoreCase(array[j].getTitle()) < 0))){
                    if(temp.getContentType().compareToIgnoreCase(array[j].getContentType()) < 0){
                        comparisonCount++;
                    }else{
                        comparisonCount += 3;
                    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        insertionSort(selectedMedia,selectedCount);
